...
class HamsterApp(QtWidgets.QMainWindow, design.Ui_MainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if DICT_HAMSTER["irods_auth"] == "pure_python_ssl":
            ...
            ssl_settings = {
                "client_server_negotiation": DICT_IRODS[
                    "irods_client_server_negotiation"
                ],
                ...
                "encryption_num_hash_rounds": int(
                    DICT_IRODS["irods_encryption_num_hash_rounds"]
                ),
    ...
    def slot_download(self):
        ...
        if current_dataobject:
            if current_dataobject.endswith("/"):
                ...
                if current_dataobject.startswith("/"):
                    ...
                    if current_dataobject == "../":
                        ...
                        fullpath_name = (
                            current_collection + "/" + current_dataobject[:-1]
                        )
            ...
            FILESIZES_SCAN_MB = FILESIZES_SCAN // 1024 // 1024
            msg = str(NFILES_SCAN) + " files, " + str(FILESIZES_SCAN_MB) + " MB"
            dst_directory = QtWidgets.QFileDialog.getExistingDirectory(
                self, "Save downloaded files (" + msg + ")", last_open_download
            )
            if dst_directory:
                ...
                if current_dataobject.endswith("/"):
                    # collection
                    self.download_collection_from_irods(
                        fullpath_name, last_open_download
                    )
                else:
        ...
        with open(dst, "wb+") as f_dst, obj.open("r") as f_src:
            ...
            while True:
                ...
                if size_tmp >= 0:
                    ...
                    print(
                        "            %d files, %.1f MB"
                        % (NFILES, size_tmp / 1024 / 1024),
                        end="\r",
                    )
            NFILES += 1
            FILESIZES += obj.size
            print(
                "            %d files, %.1f MB" % (NFILES, FILESIZES / 1024 / 1024),
                end="\r",
            )
    ...
    def zzz_download_dataobject_from_irods(self, src, dst):
        ...
        try:
            ...
            self.log_message(
                "Error reading dataobject: '" + src + "' or writing: '" + dst + "'"
            )
        return b_ok
        ...
        if selection_fullpath:
            if selection_fullpath.endswith("/"):
                ...
                try:
                    ...
                    self.log_message(
                        "Collection does not exist: '" + selection_fullpath + "'"
                    )
            else:
                # data object
                ...
                try:
                    ...
                    self.log_message(
                        "Data object does not exist: '" + selection_fullpath + "'"
                    )
            if object_exists:
                msg = ""
                try:
                ...
                if fullpath_name != new_name:
                    ...
                    if button == QtWidgets.QMessageBox.Ok:
                        ...
                        try:
                            ...
                            self.log_message(
                                "Unable to rename collection '" + fullpath_name + "'"
                            )
            else:
                # data object
                ...
                if fullpath_name != new_name:
                    ...
                    if button == QtWidgets.QMessageBox.Ok:
                        ...
                        try:
                            ...
                            self.log_message(
                                "Unable to rename dataobject: '" + fullpath_name + "'"
                            )
            ...
            if selection_fullpath.endswith("/"):
                ...
                if button == QtWidgets.QMessageBox.Ok:
                    ...
                    try:
                        ...
                        if del_coll == current_collection:
                            ...
                            update_dict(
                                DICT_HAMSTER, "current_collection", current_collection
                            )
                        self.update_collections_and_dataobjects_view(current_collection)
                    except CollectionDoesNotExist:
                        self.log_message(
                            "Unable to delete collection '" + del_coll + "'"
                        )
            else:
                # data object
                ...
                if button == QtWidgets.QMessageBox.Ok:
                    try:
                        ...
                        self.log_message(
                            "Unable to delete dataobject '" + selection_fullpath + "'"
                        )
    ...
    def slot_irods_sys_info(self):
        ...
        for res in MY_SESSION.query(UserGroup):
            msg += (
                "    " + res[UserGroup.name] + " (id: " + str(res[UserGroup.id]) + ")\n"
            )
        # print (msg)
        self.log_message(msg)
    ...
    def slot_index_changed(
        self, curr_item, prev_item
    ...
    def slot_list_item_double_clicked(self, i):
        ...
        if i.text().endswith("/"):
            ...
            if i.text().startswith("/"):
                ...
                try:
                    MY_SESSION.collections.get(
                        new_dir
                    )  # check access rights of new_dir
                    current_collection = new_dir
                    update_dict(DICT_HAMSTER, "current_collection", current_collection)
                    self.update_collections_and_dataobjects_view(current_collection)
                except CollectionDoesNotExist:
                    self.log_message(
                        "Unable to access collection for dataobject '" + i.text() + "'"
                    )
            else:
                ...
                for file_name in lst:
                    ...
                    if os.path.isfile(fullpath_name):
                        ...
                        if os.path.isdir(fullpath_name):
                            scan_ok = self.scan_directories_and_files(
                                fullpath_name, scan_ok
                            )
            except IOError:
                scan_ok = False
                self.log_message(
                    "Error: unable to scan files/directories in '" + this_dir + "'"
                )
        return scan_ok
    ...
    def upload_dirs_to_irods(self, this_dir, irods_collection):
        ...
        try:
            ...
            for file_name in lst:
                ...
                print(
                    "  %d/%d files %d%%   %d/%d directories %d%%   %d/%d MB %d%%"
                    % (
                        NFILES,
                        num_files_scan,
                        perc_f,
    ...
    def cp_form_to_dict(self, dict_this):
        ...
        dict_this.clear()
        update_dict(
            dict_this, "Hamster::contact", self.lineEdit_Hamster_contact.text().strip()
        )

        update_dict(dict_this, "dc_DOI", self.lineEdit_dc_doi.text().strip())
        update_dict(dict_this, "dc_Title", self.lineEdit_dc_title.text().strip())
        update_dict(dict_this, "dc_Creator", self.lineEdit_dc_creator.text().strip())
        update_dict(
            dict_this, "dc_Publisher", self.lineEdit_dc_publisher.text().strip()
        )
        update_dict(
            dict_this,
            "dc_Publication_year",
            self.lineEdit_dc_publication_year.text().strip(),
        )
        update_dict(
            dict_this, "dc_Resource_type", self.lineEdit_dc_resource_type.text().strip()
        )

        update_dict(dict_this, "dc_Subject", self.lineEdit_dc_subject.text().strip())
        update_dict(
            dict_this, "dc_Contributor", self.lineEdit_dc_contributor.text().strip()
        )
        update_dict(dict_this, "dc_Dates", self.lineEdit_dc_dates.text().strip())
        update_dict(
            dict_this, "dc_Related_ids", self.lineEdit_dc_related_ids.text().strip()
        )
        update_dict(
            dict_this, "dc_Description", self.lineEdit_dc_description.text().strip()
        )

        update_dict(dict_this, "dc_Language", self.lineEdit_dc_language.text().strip())
        update_dict(
            dict_this, "dc_Alternate_ids", self.lineEdit_dc_alternate_ids.text().strip()
        )
        update_dict(dict_this, "dc_Sizes", self.lineEdit_dc_sizes.text().strip())
        update_dict(dict_this, "dc_Formats", self.lineEdit_dc_formats.text().strip())
        update_dict(dict_this, "dc_Version", self.lineEdit_dc_version.text().strip())
        update_dict(
            dict_this,
            "dc_Funding_reference",
            self.lineEdit_dc_funding_reference.text().strip(),
        )
        update_dict(
            dict_this, "dc_Rights_list", self.lineEdit_dc_rights_list.text().strip()
        )
    ...
    def update_search_results(self, searchstr):
        ...
        try:
            ...
            results = (
                MY_SESSION.query(Collection, CollectionMeta)
                .filter(  # Criterion ('=', CollectionMeta.name, 'dc_Title')).filter( \
                    Criterion("like", CollectionMeta.name, "dc_%")
                )
                .filter(Criterion("like", CollectionMeta.value, searchstr))
            )
            ...
            results = (
                MY_SESSION.query(Collection.name, DataObject, DataObjectMeta)
                .filter(Criterion("like", DataObjectMeta.name, "dc_%"))
                .filter(Criterion("like", DataObjectMeta.value, searchstr))
            )
